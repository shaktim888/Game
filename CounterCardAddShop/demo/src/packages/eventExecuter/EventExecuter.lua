...
function EventExecuter:timeEvent(param)
    print("EventExecuter:timeEvent " .. param.name)
    if self:hasEvents() then
        -- 记录target
        if param.target then
            ...
            print("============ param target named of "..param.target:getName().." eventname = "..tostring(param.timename))
            self:update()
        end
    end
end
...
function EventExecuter:update(dt)
    -- print("EventExecuter:update dt = "..tostring(dt))
    local e = self:getCurEvent()
    if e then
        -- print(e:getTargetName())
