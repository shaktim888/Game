-- 场景资源加载
local LiveGameScene = class("LiveGameScene", cc.load("mvc").ModuleBase)
-- local LiveGameScene = class( "LiveGameScene",function ()
--     return cc.Scene:createWithPhysics()
-- end)

LiveGameScene.RESOURCE_FILENAME = "game/PKGS/Game.csb"
...
local GAME_OVER_TITLE  = "TIME UP!"
local COMBO_COUNT      = 3

function LiveGameScene:onCreate()
    print("你进入了游戏界面")
    -- 物件获取
    self.card      = self.mView["card"]
    self.hpBar     = self.mView["hpBar"]
    self.hpBar:setVisible(false)
    self.hpBarBg = self.mView["hpBarBg"]:setVisible(false)
...
LiveGameScene.btnBind = {
    butPause = "onButPause",
    card     = "onCard",
}

function LiveGameScene:onButPause()
...
function LiveGameScene:onCard()
    print("card")
end

-- 倒计时lastTime
function LiveGameScene:timeCountDown(time)
    print("开始倒计时：" .. time .. "s")
    self:setTime(time)
...
function LiveGameScene:setHp(val)
    --nowHp = val
    --if(val == 0) then
    --    self:onGameOver()
    --end
...
function LiveGameScene:getHp()
    return nowHp
end


function LiveGameScene:setScore(val)
    score = val
    self.textScore:setString(TEXT_SCORE .. val)
    self.textScore:runAction(self:beatAnim(1.1,0.1))
end

function LiveGameScene:getScore()
    return score
end

function LiveGameScene:setTime(val)
    lastTime = val
    self.textTime:setString(TEXT_TIME .. val)
end

function LiveGameScene:getTime()
    return lastTime
end

-- 跳动动画
function LiveGameScene:beatAnim(size,time)
...
function LiveGameScene:gotoScoreField(score)
    if(score == 0) then
        ctCombo = 0
    else
        ctCombo = ctCombo + 1
...
-- 游戏结束事件
function LiveGameScene:onGameOver()
    print("GameOver")
...
function LiveGameScene:onReplay()
    print("Replay")
    self.peachA:spawn()
...
function LiveGameScene:OnButBaseClick(but,callback)
 ...
 function LiveGameScene:saveData()
