...
function Game:initPlayerAndSenddor()
    ...
    for i= 1,#self.data do
        local player = cc.Sprite:create(resPath.."shape.png")
        player:setPosition(self.data[i].x,self.data[i].y)
        self.mView["enemyLayer"]:addChild(player)
        table.insert (self.playerArray,player)
    end
    self.pos_start = cc.p(self.playerArray[1]:getPositionX(),self.playerArray[1]:getPositionY());
    ...
    for b= 1,#self.data5 do
        ...
        send_dor:setPosition(self.data5[b].x,self.data5[b].y)
...
function Game:initBallAndGoal() 
    ...
    self.goal:setPositionX(self.size.width/2)
    self.goal:setPositionY(self.size.height-self.goal:getContentSize().height/2-40)
end

function Game:initGoalman() 
    self.goalmen = cc.Sprite:create(resPath.."people.png")
    self.goalmen:setPosition(cc.p(self.size.width/2,self.goal:getPositionY()-20))
    self.goalmen:setScale(1.5)
    self.mView["enemyLayer"]:addChild(self.goalmen)
    local a = cc.MoveBy:create(0.5,cc.p(self.goal:getContentSize().width/2,0))
    local b = cc.MoveBy:create(0.5,cc.p(-self.goal:getContentSize().width/2,0))
    ...
    if not self.ball.isOver then
        ...
        if  distance < self.ball:getBoundingBox().width + self.goalmen:getBoundingBox().width-30 and
            ...
            if self.ball:getPositionX() > self.goalmen:getPositionX() then
                self.goalmen:initWithFile(resPath.."pushright.png")
            else
                self.goalmen:initWithFile(resPath.."pushleft.png")
            end
            self.ball:runAction(cc.Sequence:create(
