...
function enemyPlayer:addPlayer()
    ...
    for i= 1,#self.data do
        local enemy = cc.Sprite:create(resPath.."people.png")
        enemy:setTag(i)
        enemy:setPosition(self.data[i].x,self.data[i].y)
...
function enemyPlayer:wall()
    ...
    for i= 1,#data do
        ...
        wall:setPosition(data[i].x,data[i].y)
...
function enemyPlayer:isDuck()
    ...
    for i= 1, #self.data4 do
        ...
        b:setPositionX(self.data4[i].x)
        b:setPositionY(self.data4[i].y)
    ...
    for i= 1, #self.data6 do
        local b = cc.Sprite:create(resPath.."missle1.png")
        b:setPosition(self.data6[i].x,self.data6[i].y)
        self.mView["allObject"]:addChild(b,5)
        b:setScale(0.5)
        b:setRotation(180);
        ...
        for a=1,#self.enemyArr do
            ...
            if distance < self.bg.ball:getBoundingBox().width/2 + 50 then
                ...
                if self.bg.ball:getPositionX() > self.enemyArr[a]:getPositionX() then
                    self.enemyArr[a]:initWithFile(resPath.."slideright.png")
                else
                    self.enemyArr[a]:initWithFile(resPath.."slideleft.png")
                end
