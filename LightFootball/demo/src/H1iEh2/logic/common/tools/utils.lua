...
function utils.hitTest(node_, point_)
    assert(node_, "hitTest node must not null!!")

    local nsp = node_:convertToNodeSpace(point_)
    local size = node_:getContentSize()
    if size.width == size.height and size.width == 0 then
        size = cc.size(node_:getLayoutSize())
    end

    local nrect = cc.rect(0,0, size.width, size.height)
    return cc.rectContainsPoint(nrect,nsp) 
end
