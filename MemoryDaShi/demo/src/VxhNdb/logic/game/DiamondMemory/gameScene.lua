...
function gameScene:grabscreen()
    ...
    self.levelNUM = math.random( 1, 10 )
...
function gameScene:init(num)
    ...
    for i = 1, num do
        for j = 1, num do
            ...
            ---png 尺寸控制：
            local sizeTable = {cc.size(166,166),cc.size(135,135),cc.size(106,106)}
            ---正面    
           
            local spStr = string.format("game/DiamondMemory/ui/%d_2.png",3); 
            local faceCard = ccui.ImageView:create(spStr)
            faceCard:setContentSize(sizeTable[num-2])
            faceCard:ignoreContentAdaptWithSize(false)
            ...
            local spStr = string.format("game/DiamondMemory/ui/%d_1.png",3);
            local backCard = ccui.ImageView:create(spStr)
            backCard:setContentSize(sizeTable[num-2])
            backCard:ignoreContentAdaptWithSize(false)
            ...
            ---对号
            local spStr = string.format("game/DiamondMemory/ui/%d_3.png",3);
            local trueIcon = ccui.ImageView:create(spStr)
            trueIcon:setContentSize(sizeTable[num-2])
            trueIcon:ignoreContentAdaptWithSize(false)
            ...
            ---错误提示
            local spStr = string.format("game/DiamondMemory/ui/%d_4.png",3);


            local falseIcon = ccui.ImageView:create(spStr)
            falseIcon:setContentSize(sizeTable[num-2])
            falseIcon:ignoreContentAdaptWithSize(false)
