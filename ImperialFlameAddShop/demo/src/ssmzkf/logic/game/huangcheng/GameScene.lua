...
function GameScene:onCreate()
    ...
    self.Xue   = self.mView['Text_1']
    self.Blood = cc.UserDefault:getInstance():getIntegerForKey("Blood")
    print("self.Blood = " .. self.Blood)
    self.Xue:setString("x " .. self.Blood)
...
function GameScene:onCollision(dt)
    ...
    elseif  (self.P_down.y < display.bottom and self.D) or self.P_left.x > display.right or self.P_right.x < display.left then
        ...
        self.Xue:setString("x " .. self.Blood)
        cc.UserDefault:getInstance():setIntegerForKey("Blood",self.Blood)
        self.U = true
        self.D = false
        self.L = true
        self.R = true
...
function GameScene:onElves()
    ...
    -- self.Panel:setSize({width = 1280.000, height = 720.000})
