...
function GameScene:onCreate()
    ...
    currentScore = 0
    global.saveTools.saveData(MONEY_SAVE_KEY, 100)
...
function GameScene:onDefenseClick(sender,type)
    if type == 2 then
        if self.isDefense and tonumber(global.saveTools.getData(MONEY_SAVE_KEY)) < 20 then
            return false
        end
...
function GameScene:onLeftAttackClick(sender,type)
    if type == 2 then
        if self.isLeftAttack and tonumber(global.saveTools.getData(MONEY_SAVE_KEY)) < 30 then
            return false
        end
...
function GameScene:onRightAttackClick(sender,type)
    if type == 2 then
        if self.isRightAttack and tonumber(global.saveTools.getData(MONEY_SAVE_KEY)) < 30 then
            return false
        end
        local score = tonumber(global.saveTools.getData(MONEY_SAVE_KEY)) - 30
        self.mView['gold_text']:setString(score)
