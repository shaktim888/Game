...
---------------  FIX TO COCOS2DX-LUA 3.15  ---------------
if cc and cc.exports then
	cc.exports.socket = {}
end
local scheduler = import("..tools.Scheduler")
function SocketTCP:dispatchEvent(data)
	if data.name == SocketTCP.EVENT_DATA then
		if type(self.__onRecv) == "function" then
			self.__onRecv(data.data,data.name)
		end
	else
		if type(self.__onEvent) == "function" then
			self.__onEvent(data.data,data.name)
		end
	end
end
function SocketTCP:onEvent(callback)
	self.__onEvent = callback
end
function SocketTCP:onRecv(callback)
	self.__onRecv = callback
end
---------------  FIX TO COCOS2DX-LUA 3.15  ---------------
...
function SocketTCP:connect(__host, __port, __retryConnectWhenFailure)
	...
	print("isipv6_only", tostring(isipv6_only))
	-- dump(addrinfo)
	if isipv6_only then
