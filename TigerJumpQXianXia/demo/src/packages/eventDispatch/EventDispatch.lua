...
function EventDispatch.addObserver(func , target)
	assert(type(func) == "function","argument 1 is type string in EventDispatch.addObserver")
...
function EventDispatch.emit(eventName, data, target)
	...
	if not target or not tolua.isnull(target) then
		for i=#listeners,1,-1 do
			if not listeners[i].target or (not tolua.isnull(listeners[i].target) and (not target or target == listeners[i].target)) then
				local flag = nil
				xpcall(function ()
	...
	for i = #observers, 1 , -1 do
		...
		if not obs.target or (not tolua.isnull(obs.target) and (not target or target == obs.target)) then
			obs.func(clone(data),eventName)
		else
