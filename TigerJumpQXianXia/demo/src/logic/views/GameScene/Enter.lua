...
local Jiesuan = require ("logic.views.GameScene.JieSuanLayer")
GameScene.btnBind = {
    back = "onBack",
    Button_4 = "Button",
    Button_5 = "Button",
...
function GameScene:onCreate(data)
    local commponentController = require("logic.viewController.componentController").new(self)
    self.defen        = 0                            --游戏得分
    self.i            = 1                            --老虎机产生的基数
    self.k            = 1                            --道具产生的基数
    ...
    audio.playMusic("game/tiger/music/1.mp3",true)
...
function GameScene:onTiger(dt)   --老虎机、道具的产生
    ...
    self.Slot_machine[self.idx] = cc.Sprite:create("game/tiger/image/tiger_ji_1.png")
    ...
    if not self.pan_tiger then
        ...
        local daoju = 1
        self.Daoju[self.idy] = cc.Sprite:create("game/tiger/daoju/daoju_1.png")
        -- local Daoju_Size = self.Daoju[self.idy]:getBoundingBox()
            ...
            self.Daoju[self.idy]:setVisible(false)
...
function GameScene:onJump() --老虎机的跳跃
    ...
    if eventType == ccui.TouchEventType.ended then
        audio.playSound("game/tiger/music/5.mp3",false)
        self.jump = true 
        self.peng = false
...
function GameScene:onPeng1(dt)
    ...
    for i, v in ipairs(daoju) do
        ...
        if (cc.rectContainsPoint(Daoju_Size,tiger_point)) or (cc.rectContainsPoint(Daoju_Size,tiger_point1)) or (cc.rectContainsPoint(Daoju_Size,tiger_point2))then
            ...
            -- print(self.defen % 5)
            -- if self.defen % 5 == 0 then
            --     local a = math.random( 1,3 )
            --     self.mView['tiger']:loadTexture("game/tiger/image/tiger_" .. a .. ".png")
            --     audio.playSound("game/tiger/music/7.wav",false)
            -- end
...
function GameScene:Button()
    ...
    local b = self.mView['Text_4']:clone()
    b:addTo(self)
    b:setVisible(true)
    b:runAction(cc.MoveTo:create(1,cc.p(640,800)))
    b:runAction(cc.FadeOut:create(1))
end

function GameScene:onClose()
    self.mView['off']:setVisible(true)
    self.mView['on']:setVisible(false)
    audio.stopMusic()
end

function GameScene:onOpen()
    self.mView['on']:setVisible(true)
    self.mView['off']:setVisible(false)
    audio.playMusic("game/tiger/music/1.mp3",true)
end
return GameScene
