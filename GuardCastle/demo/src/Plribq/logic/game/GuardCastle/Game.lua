...
function GuardCastle:onCreate()
    self:initGameInfo()
    self:onUpdate(handler(self, self.update))   
end 

function GuardCastle:initGameInfo()
    _aSaveInfo = global.saveTools.getData(cfg.GuardCastle_SAVE)
    ...
    global.saveTools.saveData(cfg.GuardCastle_SAVE, _aSaveInfo)
    self:playerAttack()
...
function GuardCastle:initSound(isOpen) 
    if not isOpen then
...
function GuardCastle:over(isSuccess)
    ...
    info.last = cfg.GAME_TIME - _iCountdown
    if info.best < cfg.GAME_TIME - _iCountdown then
        info.best = cfg.GAME_TIME - _iCountdown
    end
