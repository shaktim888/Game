...
---开始检测
--@usage g_LeakMonitor:start();  
function LeakMonitor:start()  
    self.__memLeakMonitor = self:__memLeaksMonitoring()  
end  
  
function LeakMonitor:update(dt)  
...
--返回：       无  
--增加一个受监视的表 
--@usage g_LeakMonitor:addTblToUnderMemLeakMonitor("test",tex);
function LeakMonitor:addTblToUnderMemLeakMonitor(tblName, tbl)  
...
--内存泄露监控逻辑  
function LeakMonitor:__memLeaksMonitoring()  
...
function LeakMonitor:findObject(obj, findDest,kk)  
...
function LeakMonitor:showReferences()
...
function LeakMonitor:findObjectInGlobal(obj,name)
...
function LeakMonitor:createTable()
    local proxy = {}
    local mt = {__mode = "k"}
