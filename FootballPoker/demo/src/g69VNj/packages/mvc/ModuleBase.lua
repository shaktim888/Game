    ...
    for i,module in ipairs(self.module or {}) do
        xpcall(
            function()
                ...
                self:add(m)
                print("module end create=== 2222")

                if m.export2Fmodule and type(m.export2Fmodule) == "table" then
                    for i, v in ipairs(m.export2Fmodule) do
                        -- if type(m[v]) == "function" then
                        local moriFunc = m[v]
                        local oriFunc = self[v]
                        self[v] = function (_, ...)
                            if oriFunc then
                                oriFunc(self, ...)
                            end
                ...
                print("m.exports ======== ")
                if m.exports and type(m.exports) == "table" then
                    dump(m.exports)
                    for i, v in ipairs(m.exports) do
                        local oriFunc = self[v]
                        local moriFunc = m[v]

                        if not m.export2Fmodule or (m.export2Fmodule and not table.indexof(m.export2Fmodule, v)) then
                            print( " ==========m.export2Fmodule====== ")
                            m[v] = function (_, ...)
                                local ret 
                                if moriFunc then
                                    print("moriFunc")
