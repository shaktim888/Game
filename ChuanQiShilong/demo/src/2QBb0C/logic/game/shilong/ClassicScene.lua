...
function ClassicScene:onCreate()
    ...
    --生命
    self.shengming = cc.UserDefault:getInstance():getIntegerForKey("shilong.shengming")
    dump(self.shengming)
    if self.shengming <= 0 then
        self.shengming = 3
    end
    for i = 1 , self.shengming do
        self.xue[i]:setVisible(true)
    end
    print("global.isGrabScreenMode：" .. global.isGrabScreenMode)
...
function ClassicScene:grabscreen()
    self.shengming = - 1
    cc.UserDefault:getInstance():setIntegerForKey("shilong.shengming", self.shengming)
    self.jiesuan = false
    self.zuo = false
    self.you = false
    local run1 = cc.CallFunc:create(function()
        self.LB = math.random( 0, 30 )
        self.longbi:setString( "x " .. self.LB)
        self.fen = math.random( 0, 100 )
        cc.UserDefault:getInstance():setIntegerForKey("shilong.fen",self.fen)
        self.fenshu:setString(self.fen)
        for i = 1 , 3 do
            self.xue[i]:setVisible(false)
        end
        local jiesuan = require("logic.game.shilong.JiesuanLayer").new()
        jiesuan:addTo(self)
    end)
...
function ClassicScene:onBtnBackClick()
